...
Everp의 아키텍처는 FE, Auth, Gateway, Business, SCM, Alarm을 분리한 구조였지만, 이 프로젝트를 설명할 때 더 중요한 것은 **무엇을 몇 개의 서비스로 나눴는가**보다 **권한과 계약을 어디서 고정했는가**입니다. 저희 팀은 모든 서버를 처음부터 세분 MSA로 쪼개기보다, Auth와 Gateway에서 인증/인가 책임을 먼저 고정하고, 프런트와 백엔드가 같은 응답 계약 위에서 움직이게 만든 뒤, 꼭 필요한 경계만 비동기로 분리하는 접근을 택했습니다.
...
프로젝트에서 가장 공을 많이 들인 축은 Auth와 Gateway를 분리한 OAuth 기반 인증/인가 구조였습니다. Auth에서는 Authorization Server, 클라이언트 등록, 로그인 성공 처리, JWT claims 구성을 맡고, Gateway에서는 Resource Server, JWK 기반 토큰 검증, `@PreAuthorize` 기반 권한 해석을 맡도록 역할을 분리했습니다. 이 구조는 인증 성공 여부와 업무 API 접근 허용 여부를 별도 문제로 다루게 해 주었고, 팀 전체가 권한 판단 위치를 헷갈리지 않도록 기준을 고정해 주었습니다.
...
### Problem Detail 기반 API 응답 계약

권한 경계를 고정한 뒤에는, 여러 도메인 API를 거치는 화면에서 실패를 같은 기준으로 해석할 수 있게 만드는 일이 중요했습니다. 그래서 Everp에서는 성공 응답은 화면이 바로 사용할 수 있는 DTO, 실패 응답은 Problem Detail 형식이라는 기준을 먼저 정리했습니다. 이 계약이 먼저 잡혀 있어야 Auth, Gateway, Business, SCM 어느 쪽에서 실패가 나도 프런트와 팀원이 같은 언어로 상황을 설명할 수 있습니다.

- 인증/인가 오류, 입력 오류, 도메인 규칙 오류를 상태 코드와 Problem Detail 응답 기준으로 일관되게 구분했습니다.
- Gateway와 하위 서비스가 같은 실패 해석 기준을 따르도록 정리해, 예외 처리 방식이 모듈별로 흔들리지 않게 했습니다.
- 결과적으로 권한 경계와 API 계약을 별개가 아니라 함께 고정해야 하는 기준으로 다룰 수 있었습니다.

### 선택적 비동기 연계

Kafka는 모든 서버의 기본 통신 수단이라기보다, **도메인 경계를 넘어가야 하는 일부 업무 이벤트**를 안전하게 연결하기 위한 도구로 사용했습니다. 제가 직접 커밋한 Kafka 중심 구현도 Auth, Business, SCM, Alarm 축에 집중돼 있었고, Gateway는 이 흐름의 producer/consumer가 아니라 외부 진입과 권한 해석에 집중했습니다. 따라서 이 섹션은 "전체 시스템을 내가 모두 구현했다"는 의미보다, 어떤 경계에서 비동기 연계가 필요했고 그중 어디를 직접 정리했는지를 설명하는 기록으로 보는 편이 맞습니다.
...
- **공통 오류 응답 계약**: 서비스와 프런트가 같은 기준으로 실패를 해석할 수 있도록 Problem Detail 기반 오류 응답 형식을 정리했습니다.
...
정리하면, 이 아키텍처에서 제가 가장 강하게 기여한 부분은 Auth와 Gateway 권한 경계, Problem Detail 기반 API 계약, 경계 데이터 조회 기준을 먼저 고정해 팀이 같은 기준으로 각 모듈을 구현할 수 있게 만든 점이었습니다.
