...
이 프로젝트에서는 실제 ERD를 정교하게 그리는 것보다, **어떤 서비스가 어떤 데이터를 소유하고 어떤 업무 이벤트를 처리할지**를 먼저 정리하는 것이 더 중요했습니다. ERP는 단일 엔티티보다 견적, 주문, 발주, 재고, 생산, 정산, 사용자 생성 같은 흐름이 연쇄적으로 이어지는 구조였기 때문에, 데이터 구조를 서비스 경계와 대표 트랜잭션 기준으로 함께 보는 편이 더 현실적이었습니다. 이때 팀 안에서는 Business와 SCM을 두 개의 큰 bounded context로 두고, 각 컨텍스트가 어떤 업무 언어와 조회 계약을 공유하는지부터 맞추는 접근을 택했습니다. 이 섹션은 제가 특히 많이 관여한 Auth, Gateway, 조회 계약, 초기화 데이터 기준을 중심으로 당시 팀의 데이터 경계 판단을 정리한 기록입니다.
...
모든 ERP 도메인을 처음부터 `Sales`, `Finance`, `HRM`, `MM`, `IM`, `PP` 같은 독립 서비스로 세분 MSA화하는 것이 이상적으로 보일 수는 있습니다. 하지만 당시 팀 규모와 개발 기간, 운영 복잡도를 함께 고려하면, Auth·Gateway·Alarm을 공통 축으로 두고 ERP 핵심 도메인을 Business와 SCM 두 서버로 묶는 구성이 가장 현실적이라고 판단했습니다. 이 판단은 특정 개인의 단독 설계라기보다, 팀이 설명 가능하고 구현 가능한 경계를 만들기 위한 기준 정리에 가까웠습니다. 즉, 구현 단위보다 먼저 "어떤 업무 언어를 같은 경계 안에서 설명할 수 있는가"를 맞추는 과정이었습니다.
...
도메인 데이터 구조를 설명할 때는 "어떤 테이블이 있느냐"보다 "어떤 업무 트랜잭션이 어느 서비스 사이를 건너가느냐"가 더 중요했습니다. 본편에서 중요했던 대표 흐름은 아래와 같았습니다. 이 중에서도 제가 깊게 다룬 축은 Auth 계정 생성, Gateway 뒤 조회 계약, 경계 데이터 request/reply 흐름이었습니다.
...
### 화면 중심 데이터 조합
실제 대시보드나 목록 화면에서는 한 서비스의 엔티티만으로는 충분하지 않았습니다. 그래서 원본 도메인 데이터와 별개로, 프런트가 바로 사용할 수 있는 조회 계약을 먼저 고정하는 일이 중요했습니다.
...
### 초기화 데이터 구조
프로젝트 후반에는 개발 환경에서 시나리오를 재현하기 위한 시드 구조를 강화했습니다. 이 부분은 팀이 같은 테스트 계정과 같은 업무 시나리오를 기준으로 작업할 수 있게 만드는 데 직접적인 도움이 됐습니다.
...
정리하면, 이 프로젝트의 데이터 구조는 ERP 전체를 완성도 높게 분해한 결과라기보다, 공통 인증과 업무 조회 계약을 먼저 안정시키고 팀이 같은 업무 언어와 같은 기준으로 개발할 수 있게 하기 위한 현실적인 경계 정리에 더 가까웠습니다.
