...
Everp는 FE, Auth, Gateway, Business, SCM, Alarm을 분리한 구조로 설계되었습니다. 이 구조의 핵심은 "기본 진입은 Gateway, 인증은 Auth, 실제 업무 도메인 처리는 Business와 SCM, 상태 변화 알림은 Alarm"으로 축을 명확히 두는 것이었습니다. 모든 서버를 처음부터 세분 MSA로 쪼개지는 않았지만, OAuth, 권한 경계, 응답 계약이 어디에서 다뤄져야 하는지 팀이 같은 언어로 설명할 수 있는 구조를 만드는 것이 우선 목표였습니다. 이 중에서 제가 가장 깊게 맡은 영역은 Auth와 Gateway를 기준으로 인증·인가 책임을 분리하고, 프런트와 백엔드가 같은 응답 계약 위에서 움직이게 만드는 부분이었습니다.
...
- **공통 오류 응답 계약**: 서비스와 프런트가 같은 기준으로 실패를 해석할 수 있도록 Problem Detail 기반 오류 응답 형식을 정리했습니다. 이 기준을 먼저 잡아두면 기능 구현이 늘어나도 예외 응답을 해석하는 방식이 팀 안에서 흔들리지 않습니다.
- **설명 가능한 트랜잭션**: 서비스 간 흐름은 `transactionId`와 완료 이벤트 기준으로 추적하고, 실패 시 보상 구조를 붙일 수 있게 설계했습니다.

정리하면, 이 아키텍처에서 제가 가장 강하게 기여한 부분은 공통 인증 서버, Gateway 권한 해석, 경계 데이터 조회, 응답 계약을 먼저 고정해 팀이 같은 기준으로 각 모듈을 구현할 수 있게 만든 점이었습니다.
