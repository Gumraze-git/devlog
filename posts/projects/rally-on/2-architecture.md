...
RallyOn에서 아키텍처를 설명할 때 가장 중요한 것은 "어떤 패턴을 썼는가"보다 **도메인 경계와 인증 책임을 어디서 나눴는가**입니다. 이 프로젝트에서는 먼저 사용자, 자유게임, 외부 의존을 구조 안에서 분리했고, 그 위에서 브라우저에서 보이는 로그인 화면과 서버가 맡아야 하는 인가 코드 처리·토큰 검증·재발급을 역할별로 나누는 판단이 이어졌습니다. 이후 그 판단을 OAuth 2.1 기반 인증·인가 구조와 reverse proxy 기반 브라우저 인증 흐름까지 연결해 구현했습니다.
...
RallyOn에서 가장 큰 구조 전환 중 하나는 토큰을 localStorage에 두지 않고 **HttpOnly + Secure 쿠키**로 다루는 동시에, 인증과 인가 책임을 다시 나누는 것이었습니다. 로컬 로그인 도입 이후 계정, 세션, 토큰 책임이 한 레이어에 붙기 시작하면 인증 변경이 비즈니스 도메인까지 번지기 쉬웠기 때문에, 단일 애플리케이션 안에서도 Authorization 역할과 Resource 역할을 구분해 설명 가능한 구조로 정리했습니다.
...
- 로그인 화면 전환은 프런트가 담당하고, 인가 코드 교환·토큰 검증·재발급은 서버가 담당하도록 역할을 나눴습니다.
- 이 구조 덕분에 인증 성공 여부와 비즈니스 도메인 접근 규칙을 한 레이어에서 섞지 않고 설명할 수 있게 됐습니다.

### reverse proxy 기반 브라우저 인증 흐름 설계

인증 책임을 정리한 뒤에도, secure cookie 기반 흐름을 로컬에서 실제처럼 검증하지 못하면 브라우저 인증은 계속 추상적인 설계에 머물 수밖에 없습니다. 그래서 `localhost` 포트 조합을 우회하는 대신, `auth.rallyon.test`와 `api.rallyon.test`, nginx proxy, mkcert, DUMMY provider를 기준으로 브라우저 실행 경계를 먼저 고정했습니다.
...
- 프런트는 로그인 UI와 사용자 여정을 담당하고, 백엔드는 인가 코드 교환, 토큰 검증, 재발급 같은 인증 규칙을 담당하는 방식으로 역할을 나눴습니다.

이 실행 경계를 먼저 고정한 덕분에 브라우저 기준 인증 흐름을 실제와 비슷한 조건에서 반복 검증할 수 있었고, reverse proxy가 단순 인프라 설정이 아니라 인증 책임 분리를 검증하는 실행 환경 역할을 하게 됐습니다.
