...
RallyOn의 데이터 구조는 "배드민턴 앱"이라는 이름보다 **운영 단위가 무엇인가**를 먼저 반영하도록 설계돼 있습니다. 중요한 것은 게시글이나 피드보다, 하나의 자유게임 세션을 생성하고 참가자를 관리하고 라운드별 매치를 편성하는 흐름이었습니다.
...
이걸 하나의 거대한 요청으로 묶어 두면 수정 충돌도 많아지고 검증도 어려워집니다. 그래서 RallyOn은 세션 본체와 편성 데이터를 나눠, 운영자가 어떤 단위를 수정하는지 API 레벨에서도 명확하게 유지했습니다.
...
자유게임 생성 단계에서는 여기에 별도로 **장소 검색**이 연결됩니다. 장소 자체를 거대한 마스터 테이블로 먼저 들고 가기보다, 운영 시점에 검색 결과를 받아 세션의 location에 반영하는 방식으로 단순화했습니다.
