---
title: "영화 리뷰 및 추천 웹 어플리케이션 개발"
summary: "영화 리뷰 및 리뷰 기반 영화 추천 서비스"
project_title: "Inside Movie"
...
### 트러블슈팅
- **문제**: 인증 토큰 처리와 권한 검사 로직이 기능별로 분산되어 예외 응답 형식이 일관되지 않음
- **원인**: 필터 체인/예외 처리 책임이 명확히 정리되지 않아 엔드포인트별 응답 형태가 달라짐
- **해결**: 인증 예외 처리 지점을 필터로 모으고 공통 에러 응답 포맷 적용
- **결과**: 프론트엔드 연동 시 인증 실패 케이스 처리 단순화, 디버깅 시간 단축
...
### 트러블슈팅
- **문제**: 초기 구현에서 엔티티/DTO 경계가 불명확해 API 수정 시 파급 범위가 큼
- **원인**: Controller에서 엔티티를 직접 다루며 계층 책임이 혼재됨
- **해결**: DTO 매핑 지점을 명확히 분리하고 서비스 레이어 중심으로 도메인 로직 재배치
- **결과**: 기능 확장 시 변경 영향 범위가 줄고 코드 가독성/유지보수성 향상
...
### 트러블슈팅
- **문제**: 추천 응답에서 FastAPI 호출 구간 지연으로 체감 응답 속도 저하
- **원인**: 외부 연동 구간의 실패/지연 대비 전략(타임아웃, 폴백)이 부족함
- **해결**: 호출 타임아웃/예외 처리 적용 및 분석 실패 시 기본 추천 로직으로 폴백
- **결과**: 감정 분석 서비스 상태와 무관하게 추천 API 가용성 유지
